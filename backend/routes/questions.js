...
router.post('/', [
  ...
  body('studentName').trim().notEmpty().withMessage('Student name is required')
    .isLength({ min: 1, max: 50 }).withMessage('Student name must be between 1 and 50 characters'),
  ...
  try {
    ...
    const session = await Session.findOne({ sessionId }).populate('course', 'title code');
    if (!session) {
...
router.get('/session/:sessionId', [
  query('status').optional().isIn(['all', 'answered', 'unanswered', 'important'])
    .withMessage('Status must be one of: all, answered, unanswered, important'),
...
router.put('/:id/status', [
  body('action').isIn(['toggle_answered', 'toggle_important', 'mark_answered', 'mark_important', 'unmark_answered', 'unmark_important'])
